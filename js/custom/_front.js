...
jQuery(document).ready(function () {
	...
	jQuery('.swiper-slide.swiper-slide-duplicate').each(function(){
		var heightSwiperSlide = jQuery(this).data('autoheight'),
			heightSwiperSlideImg = jQuery(this).children('img').height();

			if (heightSwiperSlide < heightSwiperSlideImg) {
				jQuery(this).height(heightSwiperSlideImg);
...
function calcSizeIsotope(cont) {
	"use strict";
	var columns = jQuery(window).width() < 480 ? 1 : Math.max(1, Number(cont.data('columns')));
